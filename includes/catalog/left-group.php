<?php
class BMW_Left_group{
	use Letscms_BMW_CommonClass;

	public function bmw_myleft_group($userKey){
		global $wpdb;
		if(isset($userKey))
			{
	...
	public function view_leftgroup(){
		global $wpdb;
		$data = array();
		$key = $this->get_current_user_key();
		$user_id = $this->getUserIdByKey($key); 
		
		$data 	= $this->bmw_myleft_group($key); 
	 	$totalArr 	= $this->MyDirectGroupTotal($key); 

?>
<div class="container">
	<h2><?php echo __('Left Group Details','bmw');?></h2>
	<?php $this->letscms_check_user(); ?>
	<div class="row" >
		<div class="col-sm-4"><?php echo __('Total Left downlines : ','bmw');?><?php echo $totalArr['left']; ?></div>
	</div>
	<div id="clear"></div>
	<div class="row">
		<div class="col-sm-12">
			<table class="table table-hover">
				<thead>
				    <tr>
				      ...
				      <th scope="col"><?php echo __('Left Member','bmw');?></th>
